  ...
  useEffect(() => {
    ...
    const enter: any[] = [
      [
        ...
        { duration: 0.45, ease: [0.65, 0, 0.35, 1], type: 'spring', stiffness: 20, restDelta: 2 }
      ],
      ['.line', { width: '100%' }],
      ['.heading', { y: '0%' }, { duration: 0.3, delay: stagger(0.0185), ease: 'easeInOut' }],
      ['.description', { opacity: 1, y: 0 }]
    ];

    const exit: any[] = [
      ['.description', { opacity: 0, y: 20 }],
      ['.heading', { y: '100%' }, { duration: 0.3, delay: stagger(0.0185), ease: 'easeInOut' }],
      ['.line', { width: '0%' }],
      [
        containerRef.current,
